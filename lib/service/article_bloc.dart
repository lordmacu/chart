...
class ArticleBloc {
  ...
  // Inputs
  Future<void> getArticles({bool refresh = false, String param}) async {
  ...
  Future<List<Article>> _getArticles(
      {int page = 1, int pageSize = _pageSize,String param}) async {
    String jsonString = await api.getArticles(
        sources: _activeSourcesStr, page: page, pageSize: pageSize,param: param);
    if (jsonString != null) {
      var data = json.decode(jsonString);
      if (data != null) {
        _totalItemsForRequestedSources = 100;
        List<Article> articles = (data["data"] as List<dynamic>)
